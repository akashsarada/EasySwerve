...
public class DriveTrain extends SubsystemBase {
    ...
    public double fieldCentrifyStrafe (double stickY, double stickX, double robotAngle){
        ...
        return  (stickX * Math.cos(robotAngle)) - (stickY * Math.sin(robotAngle)) ;
    ...
    private double[] calculateCoefficients (double forward, double strafe, double turn) {
        ...
        coefficients[0] = strafe - (turn * (Constants.RobotParameters.ROBOT_LENGTH / Constants.RobotParameters.ROBOT_DIAMETER));
        coefficients[1] = strafe + (turn * (Constants.RobotParameters.ROBOT_LENGTH / Constants.RobotParameters.ROBOT_DIAMETER));
        coefficients[2] = forward - (turn * (Constants.RobotParameters.ROBOT_WIDTH / Constants.RobotParameters.ROBOT_DIAMETER));
        coefficients[3] = forward + (turn * (Constants.RobotParameters.ROBOT_WIDTH / Constants.RobotParameters.ROBOT_DIAMETER));

        return coefficients;
    }
    private double[] calculatePower(double[] coefficients, double nerf) {
        ...
        power[2] = Math.sqrt(Math.pow(coefficients[0], 2) + Math.pow(coefficients[2], 2));
        power[3] = Math.sqrt(Math.pow(coefficients[0], 2) + Math.pow(coefficients[3], 2));

        double maxPower = Math.max(Math.max(power[0], power[2]), Math.max(power[3], power[1]));

        if (maxPower != 0){
            power[0] = (power[0] / maxPower) * nerf;
            power[1] = (power[1] / maxPower) * nerf;
            power[2] = (power[2] / maxPower) * nerf;
            power[3] = (power[3] / maxPower) * nerf;
    ...
    private double[] calculateAngle(double[] coefficients) {
        ...
        angle[2] = Math.toDegrees(Math.atan2(coefficients[0], coefficients[2]));
        angle[3] = Math.toDegrees(Math.atan2(coefficients[0], coefficients[3]));
    ...
    public void setSwerve(double forward, double strafe, double turn) {
        ...
        setPower(calculatePower(coefficients, 0.3));
